...
**** DONE special folder and file
** adopt the existing folked code and test again
*** DONE fix warnings
    commented out mu4e; commented out some code for older
versions; changed cl to be cl-lib; commented out the lisp stuff.
...
** Special files and directories
    ...
    There are some special files and directoies in the inital configurations.

    =private.el=: keeps some private settings.

    =site-lisp=: manually installed extensions.
 
    =agenda.org=: agenda file used by Org mode.

** Others
   
    - Comment out a region: select a region and use command =M-x comment-region= 
...
** Packages
   ...
   Changed to use =cl-lib= instead of =cl= to remove the warning, because =cl=
   is not recommended to use after Emacs 24. This [[http://stackoverflow.com/questions/5019724/in-emacs-what-does-this-error-mean-warning-cl-package-required-at-runtime#5020049][StackOverflow post]] has more
   explanations.
...
** Modes
  ...
  ; this can be deleted as I use ver. 25+
